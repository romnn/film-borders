pub fn oldapply(&mut self, options: ImageBorderOptions) -> Result<RgbaImage, ImageError> {
        let mut size = Size {
        ...
        let photo = self.img.buffer.clone();
        ...
        let filter_type = FilterType::Triangle;
        photo = resize(&photo, fit_width, fit_height, filter_type);
        println!(
            "fitting to {} x {} took {:?}",
            fit_width,
            fit_height,
        ...
        let start = Instant::now();
        let filter_type = FilterType::Triangle;
        fb = resize(&fb, fb_width, fb_height, filter_type);
        println!(
